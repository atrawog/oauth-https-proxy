...
def main(
    ...
    # We need a custom ASGI middleware that intercepts /mcp and /mcp/ paths
    async def mcp_handler(scope, receive, send):
        """ASGI handler that routes /mcp requests without redirects"""
        if scope["type"] == "http":
            path = scope.get("path", "")
            if path == "/mcp" or path == "/mcp/":
                # Handle the request directly without redirects
    ...
    # Replace the app with our custom handler
