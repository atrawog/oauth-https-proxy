...
cert-create name domain staging="false" email=env_var_or_default("ADMIN_EMAIL", "") token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client cert create {{name}} {{domain}} \
        {{ if staging == "true" { "--staging" } else { "" } }} \
        --email {{email}}

# List all certificates
cert-list token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client cert list

# Show certificate details
cert-show name pem="false" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client cert show {{name}} \
        {{ if pem == "true" { "--pem" } else { "" } }}

# Delete a certificate
cert-delete name force="true" token=env_var("OAUTH_ACCESS_TOKEN"):
    TOKEN={{token}} pixi run proxy-client cert delete {{name}} --force

# Renew a certificate
cert-renew name force="true" wait="true" token=env_var("OAUTH_ACCESS_TOKEN"):
    TOKEN={{token}} pixi run proxy-client cert renew {{name}} --force \
        {{ if wait == "false" { "--no-wait" } else { "" } }}

# Convert staging certificate to production
cert-convert-to-production name wait="true" force="true" token=env_var("OAUTH_ACCESS_TOKEN"):
...
proxy-create hostname target-url staging="false" preserve-host="true" enable-http="true" enable-https="true" email=env_var_or_default("ADMIN_EMAIL", "") token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
...
# List all proxies
proxy-list token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client --format table proxy list

# Show proxy details
proxy-show hostname token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client proxy show {{hostname}}

# Delete a proxy
proxy-delete hostname delete-cert="false" force="true" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
proxy-auth-enable hostname auth-proxy="auth.localhost" mode="forward" allowed-scopes="" allowed-audiences="" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
proxy-auth-disable hostname token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client proxy auth disable {{hostname}}

# Configure authentication for a proxy
proxy-auth-config hostname users="" emails="" groups="" allowed-scopes="" allowed-audiences="" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
proxy-auth-show hostname token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client proxy auth show {{hostname}}

# Set protected resource metadata
proxy-resource-set hostname endpoint="/api" scopes="read,write" stateful="false" override-backend="false" bearer-methods="header" doc-suffix="/docs" server-info="{}" custom-metadata="{}" hacker-one-research="" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
proxy-resource-show hostname token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client proxy resource show {{hostname}}

# Clear protected resource metadata
proxy-resource-clear hostname token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client proxy resource clear {{hostname}} --force

# List all protected resources
proxy-resource-list token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
proxy-oauth-server-set hostname issuer="" scopes="" grant-types="" response-types="" token-auth-methods="" claims="" pkce-required="false" custom-metadata="{}" override-defaults="false" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
proxy-oauth-server-show hostname token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client proxy oauth-server show {{hostname}}

# Clear OAuth server configuration for a proxy
proxy-oauth-server-clear hostname token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
proxy-oauth-server-list token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
proxy-github-oauth-set hostname client-id client-secret token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    #!/usr/bin/env bash
    set -euo pipefail
...
proxy-github-oauth-show hostname token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    #!/usr/bin/env bash
    set -euo pipefail
    API_URL="${API_URL:-http://localhost:80}"
    TOKEN="${TOKEN:-{{token}}}"
...
proxy-github-oauth-clear hostname token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    #!/usr/bin/env bash
    set -euo pipefail
    API_URL="${API_URL:-http://localhost:80}"
    TOKEN="${TOKEN:-{{token}}}"
...
proxy-github-oauth-list token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    #!/usr/bin/env bash
    set -euo pipefail
    API_URL="${API_URL:-http://localhost:80}"
    TOKEN="${TOKEN:-{{token}}}"
...
# Create a new route
route-create path target-type target-value priority="50" methods="ALL" is-regex="false" description="" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
route-create-global path target-type target-value priority="50" methods="*" is-regex="false" description="" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
route-create-proxy path target-type target-value proxies priority="500" methods="*" is-regex="false" description="" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
# List all routes
route-list token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client route list

# Show route details
route-show route-id token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client route show {{route-id}}

# Delete a route
route-delete route-id token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client route delete {{route-id}} --force

# List routes by scope
route-list-by-scope scope="all" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
service-create name image="" dockerfile="" port="" memory="512m" cpu="1.0" auto-proxy="false" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
service-create-exposed name image port bind-address="127.0.0.1" memory="512m" cpu="1.0" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service create-exposed {{name}} {{image}} {{port}} \
        --bind-address {{bind-address}} \
        --memory {{memory}} --cpu {{cpu}}

# List Docker services
service-list owned-only="false" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client service list --type docker

# List all services (Docker + external)
service-list-all type="" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client service list \
        {{ if type != "" { "--type " + type } else { "--type all" } }}

# Show service details
service-show name token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service show {{name}}

# Delete a service
service-delete name force="true" delete-proxy="true" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service delete {{name}} --force

# Start a service
service-start name token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service start {{name}}

# Stop a service
service-stop name token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service stop {{name}}

# Restart a service
service-restart name token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service restart {{name}}

# View service logs
service-logs name lines="100" timestamps="false" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service logs {{name}} --lines {{lines}}

# Show service statistics
service-stats name token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service stats {{name}}

# Create proxy for service
service-proxy-create name hostname="" enable-https="false" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
# Clean up services
service-cleanup token=env_var("OAUTH_ACCESS_TOKEN"):
    TOKEN={{token}} pixi run proxy-client service cleanup --force

# Clean up orphaned services
service-cleanup-orphaned token=env_var("OAUTH_ACCESS_TOKEN"):
    TOKEN={{token}} pixi run proxy-client workflow cleanup --orphaned-only --force

# Add port to service
service-port-add name port bind-address="127.0.0.1" source-token="" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
service-port-remove name port-name token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service port remove {{name}} {{port-name}} --force

# List service ports
service-port-list name token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service port list {{name}}

# Check port availability
service-port-check port bind-address="127.0.0.1" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service port check {{port}} --bind-address {{bind-address}}

# List global port allocation
service-ports-global available-only="false" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service ports \
        {{ if available-only == "true" { "--available-only" } else { "" } }}

# Register external service
service-register name target-url description="" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service external register {{name}} {{target-url}} \
        {{ if description != "" { "--description '" + description + "'" } else { "" } }}

# Unregister external service
service-unregister name token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service external unregister {{name}} --force

# List external services
service-list-external token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client service external list

# Show external service details
service-show-external name token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service external show {{name}}

# Update external service
service-update-external name target-url description="" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client service external update {{name}} {{target-url}} \
        {{ if description != "" { "--description '" + description + "'" } else { "" } }}

# Register OAuth as external service
service-register-oauth token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
oauth-clients-list active-only="" page="1" per-page="50" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
oauth-token-list token_type="" client_id="" username="" page="1" per_page="50" include_expired="false" token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
# List OAuth sessions
oauth-sessions-list token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    TOKEN={{token}} pixi run proxy-client oauth session list

# Generate OAuth JWT key (special case - uses openssl)
oauth-key-generate token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
    #!/usr/bin/env bash
    set -euo pipefail
...
# Test OAuth tokens
oauth-test-tokens server-url token=env_var_or_default("OAUTH_ACCESS_TOKEN", ""):
...
# Search logs
logs hours="1" event="" level="" hostname="" limit="50" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log search \
        --hours {{hours}} --limit {{limit}} \
        {{ if hostname != "" { "--hostname " + hostname } else { "" } }}

# Query logs by client IP
logs-ip ip hours="1" event="" level="" limit="100" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log ip {{ip}} --hours {{hours}} --limit {{limit}}

# Query logs by proxy hostname
logs-proxy hostname hours="1" limit="100" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
...
logs-hostname hostname hours="1" limit="100" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log hostname {{hostname}} --hours {{hours}} --limit {{limit}}

# Query logs by OAuth client ID
logs-oauth-client client-id hours="1" event="" level="" limit="100" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log oauth-client {{client-id}} --hours {{hours}} --limit {{limit}}

# Show errors
logs-errors hours="1" limit="20" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log errors --hours {{hours}} --limit {{limit}}

# Debug errors
logs-errors-debug hours="1" include-warnings="false" limit="50" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log errors --hours {{hours}} \
        {{ if include-warnings == "true" { "--include-warnings" } else { "" } }} \
        --limit {{limit}}

# Follow logs
logs-follow interval="2" event="" level="" hostname="" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log follow --interval {{interval}} \
        {{ if hostname != "" { "--hostname " + hostname } else { "" } }}

# OAuth activity for an IP
logs-oauth ip hours="1" limit="100" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log oauth {{ip}} --hours {{hours}} --limit {{limit}}

# OAuth debugging for an IP
logs-oauth-debug ip hours="1" limit="100" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log oauth-debug {{ip}} --hours {{hours}} --limit {{limit}}

# OAuth flow tracking
logs-oauth-flow client-id="" username="" hours="1" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
...
logs-search query="" hours="1" event="" level="" hostname="" limit="100" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log search \
...
# Log statistics
logs-stats hours="1" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log stats --hours {{hours}}

# Clear logs
logs-clear token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log clear --force

# Test logging system
logs-test token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log test

# Query logs by authenticated user ID
logs-user user-id hours="1" limit="100" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log user {{user-id}} --hours {{hours}} --limit {{limit}}

# Query logs by session ID
logs-session session-id hours="1" limit="100" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log session {{session-id}} --hours {{hours}} --limit {{limit}}

# Query logs by HTTP method
logs-method method hours="1" limit="100" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log method {{method}} --hours {{hours}} --limit {{limit}}

# Query logs by status code
logs-status code hours="1" limit="100" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log status {{code}} --hours {{hours}} --limit {{limit}}

# Query slow requests
logs-slow threshold-ms="1000" hours="1" limit="50" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log slow --threshold {{threshold-ms}} --hours {{hours}} --limit {{limit}}

# Query logs by path pattern
logs-path pattern hours="1" limit="100" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
    TOKEN={{token}} pixi run proxy-client log path "{{pattern}}" --hours {{hours}} --limit {{limit}}

# Query logs by OAuth username
logs-oauth-user username hours="1" limit="100" token=env_var_or_default("OAUTH_ACCESS_TOKEN", "acm_public"):
