...
class AsyncRedisStorage:
    ...
    async def store_api_token(self, name: str, token: str, cert_email: Optional[str] = None) -> bool:
        ...
        try:
            ...
            result2 = await self.redis_client.hset(name_key, mapping={
                "name": name,
                "hash": token_hash,
                "token": token,
    ...
    async def delete_api_token(self, token_hash: str) -> bool:
        ...
        try:
            ...
            auth_key = f"auth:token:{token_hash}"
            token_json = await self.redis_client.get(auth_key)
            ...
            if token_json:
                ...
                # Delete both keys
                result1 = bool(await self.redis_client.delete(auth_key))
                result2 = True
                if name:
                    name_key = f"token:{name}"
                    result2 = bool(await self.redis_client.delete(name_key))
    ...
    async def delete_api_token_by_name(self, name: str) -> bool:
        ...
        try:
            ...
            name_key = f"token:{name}"
            token_data = await self.redis_client.hgetall(name_key)
    ...
    async def delete_api_token_cascade(self, token_hash: str) -> dict:
        ...
        try:
            ...
            cert_cursor = 0
            while True:
                cert_cursor, cert_keys = await self.redis_client.scan(
                ...
                for cert_key in cert_keys:
                    ...
                    if cert_key.startswith("cert:domain:"):
                        continue
                    cert_json = await self.redis_client.get(cert_key)
                    if cert_json:
                        cert = json.loads(cert_json)
                        if cert.get('owner_token_hash') == token_hash:
                            if await self.redis_client.delete(cert_key):
            ...
            proxy_cursor = 0
            while True:
                proxy_cursor, proxy_keys = await self.redis_client.scan(
                    proxy_cursor, match="proxy:*", count=100
                )
                for proxy_key in proxy_keys:
                    proxy_json = await self.redis_client.get(proxy_key)
                    if proxy_json:
                        proxy = json.loads(proxy_json)
                        if proxy.get('owner_token_hash') == token_hash:
                            if await self.redis_client.delete(proxy_key):
            ...
            stats['token_deleted'] = await self.delete_api_token(token_hash)
    ...
    async def delete_api_token_cascade_by_name(self, name: str) -> dict:
        ...
        try:
            ...
            token_hash = token_data.get('hash')
            if token_hash:
                return await self.delete_api_token_cascade(token_hash)
    ...
    async def update_api_token_email(self, token_hash: str, cert_email: str) -> bool:
        ...
        try:
            ...
            # Update in auth key
            result1 = await self.redis_client.set(auth_key, json.dumps(token_data))
            
            # Update in name key
            name = token_data.get('name')
            if name:
                name_key = f"token:{name}"
                result2 = await self.redis_client.hset(name_key, 'cert_email', cert_email)
                return result1 and result2
