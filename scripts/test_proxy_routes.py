...
def test_route_modes():
    ...
    resp = requests.get(f"{BASE_URL}/proxy/targets/route-test.example.com/routes")
    data = resp.json()
...
def test_selective_routes():
    ...
    resp = requests.get(f"{BASE_URL}/proxy/targets/selective-test.example.com/routes")
    data = resp.json()
...
def test_disabled_routes():
    ...
    # Get initial routes
    resp = requests.get(f"{BASE_URL}/proxy/targets/disable-test.example.com/routes")
...
def test_bulk_route_set():
    ...
    if len(all_routes) >= 2:
        ...
        # Verify
        resp = requests.get(f"{BASE_URL}/proxy/targets/bulk-test.example.com/routes")
        data = resp.json()
