...
# Test SSE endpoint
api_url = "http://fetcher.atradev.org/sse"
...
with httpx.Client() as client:
    try:
        ...
        resp = client.get(f"{api_url}?sessionId={session_id}", headers=headers, timeout=5.0)
    ...
    try:
        resp = client.post(f"{api_url}?sessionId={session_id}", 
                          json=init_msg,
