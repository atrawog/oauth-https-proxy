...
def main():
    if len(sys.argv) < 4:
        print("Usage: proxy_route_enable.py <hostname> <route-id> <token>")
        sys.exit(1)
    ...
    url = f"{base_url}/proxy/targets/{hostname}/routes/{route_id}/enable"
    ...
    try:
        ...
        print(f"Route '{route_id}' enabled for {hostname}")
        ...
        if routes_resp.ok:
            ...
            if routes_data['route_mode'] == 'selective':
                print(f"  Enabled routes: {', '.join(routes_data['enabled_routes'])}")
            elif routes_data['route_mode'] == 'all':
                disabled = routes_data['disabled_routes']
                if route_id in disabled:
                    print(f"  Route was removed from disabled list")
                print(f"  Disabled routes: {', '.join(disabled) if disabled else 'None'}")
