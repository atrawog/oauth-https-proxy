...
try:
    response = requests.get(f"{API_URL}/token/info", headers=headers, verify=False, timeout=5)
...
try:
    response = requests.get(f"{API_URL}/api/v1/certificates", headers=headers, verify=False, timeout=5)
...
response = requests.get(f"{API_URL}/api/v1/certificates", headers=headers, verify=False, allow_redirects=False)
